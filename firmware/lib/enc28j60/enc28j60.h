...
namespace Enc28j60
{
  ...
  // - CS must be activated 50ns before transfer start and stay active 10ns (210ns if delay == true) after transfer end, then deactivated.
  ...
  //     - txRxLen is in range [1, 3]
  //     - txlen and rxlen is in range [1, ~1520]
  ...
  // - Implementation must be conditionally safe: 
  //     - methods of different objects may be called simultaneously from different threads
  //     - methods of same object may be called from different threads, but are guaranteed not to be called simultaneously
  class Spi : mstd::noncopyable
  {
  public:
    virtual ~Spi() = default;
    virtual int reinit() = 0;
    virtual int txRx(uint8_t* txRx, size_t txrxLen, bool delay) = 0;
